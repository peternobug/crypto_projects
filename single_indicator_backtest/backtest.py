...
class Backtest:
    ...
    def signal_generator(self, i: int):
        ...
        :return:
        """
        rsi = self.df.loc[i, self.strategy_name.lower()]
        open_long_signal = rsi < self.first_window
        open_short_signal = rsi > self.second_window
        close_long_signal = rsi > self.second_window
        close_short_signal = rsi < self.first_window
    ...
    def run(self):
        ...
        start_index = self.reference_window
        for i in range(start_index, len(self.df)):
            current_datetime = self.df.loc[i, 'date_time']
            now_close = self.df.loc[i, "close"]
            if (current_datetime.hour >= self.start_time) & (current_datetime.hour <= (self.end_time - 1)) & \
            ...
            if pos_opened:
                ...
                if close_long_signal and (holding_position == 'LONG'):
                    holding_position = None
                    pos_opened = False
                    temp_capital += (now_close - open_price) * num_of_lot - num_of_lot * now_close * self.taker_fee
                    ...
                    holding_position = None
                    pos_opened = False
                    temp_capital += (open_price - now_close) * num_of_lot - num_of_lot * now_close * self.taker_fee
        ...
        sharpe_ratio = return_mean_pct / return_sd_pct * math.sqrt(252)
        ...
        pprint(
            f"{sharpe_ratio = }, {mdd_pct = } , {sortino_ratio = } , {calmar_ratio = } , "
            f"{net_profit = }, {win_rate = }, {num_of_trade = }")
...
if __name__ == "__main__":
    symbol = "BTCUSDT"
    para_dict = {"strategy_name": "RSI", "first_window": 30, "second_window": 70,
                 "reference_window": 20, "sub_window_multiplier": 8, "stop_loss": 1, "backtest_year": 1,
                 "invest_percentage": 50, "symbol": symbol, "start_time": 8, "end_time": 17}
    interval = "1m"
